			...
			if(ren!=null){
				...
				if(trans.GetComponent<Renderer>()!=null && trans.GetComponent<Renderer>().material.HasProperty("_Color")){
					...
					for ( int i = 0; i < trans.childCount; i++) {
						if(trans.GetChild(i).gameObject.GetComponent<Renderer>()!=null){
							Color col = trans.GetChild(i).gameObject.GetComponent<Renderer>().material.color;
							this.fromInternal.x = col.a;
							break;
						}
					}
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
		...
		if(renderer!=null){
			for ( int i = 0; i < renderer.materials.Length; i++){
				if(renderer.materials[i].HasProperty("_Color")){
                    renderer.materials[i].color = new Color(renderer.materials[i].color.r, renderer.materials[i].color.g, renderer.materials[i].color.b, val);
				}else if(renderer.materials[i].HasProperty("_TintColor")){
					Color col = renderer.materials[i].GetColor ("_TintColor");
                    renderer.materials[i].SetColor("_TintColor", new Color( col.r, col.g, col.b, val));
				}
			}
		}
	...
	private static void colorRecursive( Transform transform, Color toColor, bool useRecursion = true ){
		...
		if(ren!=null){
			for ( int i = 0; i < ren.materials.Length; i++){
                ren.materials[i].color = toColor;
			}
		}
		if(useRecursion && transform.childCount>0){
            for (int i = 0; i < transform.childCount; i++)
            {
				colorRecursive(transform.GetChild(i), toColor);
			}
		}
	}
	...
	private static void alphaRecursive( RectTransform rectTransform, float val, int recursiveLevel = 0){
		if(rectTransform.childCount>0){
			for ( int i = 0; i < rectTransform.childCount; i++) {
				UnityEngine.UI.MaskableGraphic uiImage = rectTransform.GetChild(i).GetComponent<UnityEngine.UI.Image>();
				if (uiImage != null) {
					Color c = uiImage.color; c.a = val; uiImage.color = c;
				} else {
					uiImage = rectTransform.GetChild(i).GetComponent<UnityEngine.UI.RawImage>();
					if (uiImage != null) {
						Color c = uiImage.color; c.a = val; uiImage.color = c;
					}
				}

				alphaRecursive(rectTransform.GetChild(i).GetComponent<RectTransform>(), val, recursiveLevel + 1);
			}
		}
	}
	...
	private static void alphaRecursiveSprite( Transform transform, float val ){
		if(transform.childCount>0){
			for ( int i = 0; i < transform.childCount; i++) {
				SpriteRenderer ren = transform.GetChild(i).GetComponent<SpriteRenderer>();
				if(ren!=null)
					ren.color = new Color( ren.color.r, ren.color.g, ren.color.b, val);
				alphaRecursiveSprite(transform.GetChild(i), val);
			}
		}
	}
	...
	private static void colorRecursiveSprite( Transform transform, Color toColor ){
		if(transform.childCount>0){
            for (int i = 0; i < transform.childCount; i++)
            {
				...
				if(ren!=null)
					ren.color = toColor;
				colorRecursiveSprite(transform.GetChild(i), toColor);
			}
		}
	}
	...
	private static void colorRecursive( RectTransform rectTransform, Color toColor ){
		...
		if(rectTransform.childCount>0){
			for (int i = 0; i < rectTransform.childCount; i++) {
				...
				colorRecursive(rectTransform.GetChild(i), toColor);
			}
		}
	}
	...
	private static void textAlphaChildrenRecursive( Transform trans, float val, bool useRecursion = true ){
		...
		if(useRecursion && trans.childCount>0){
			for ( int i = 0; i < trans.childCount; i++) {
				UnityEngine.UI.Text uiText = trans.GetChild(i).GetComponent<UnityEngine.UI.Text>();
				if(uiText!=null){
					Color c = uiText.color;
					c.a = val;
					uiText.color = c;
				}
				textAlphaChildrenRecursive(trans.GetChild(i), val);
			}
		}
	}
	...
	private static void textAlphaRecursive( Transform trans, float val, bool useRecursion = true ){
		...
		if(useRecursion && trans.childCount>0){
			for (int i = 0; i < trans.childCount; i++) {
				textAlphaRecursive(trans.GetChild(i), val);
			}
		}
	}
	...
	private static void textColorRecursive(Transform trans, Color toColor ){
		if(trans.childCount>0){
			for (int i = 0; i < trans.childCount; i++) {
				...
				textColorRecursive(trans.GetChild( i ), toColor);
			}
		}
	}
	#endif
